...
import (
	...
	"sre-tool/internal/container"
	"sync"
	"time"
)

type Config struct {
	Driver container.IDriver
	cache  container.ICache
	mu     *sync.Mutex
}

var _ container.IConfig = (*Config)(nil)
...
// New config
func New(config container.IDriver, option Options) (provider *Config, err error) {
	if option.Cache == nil {
